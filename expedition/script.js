...
function expedition_startNegotiaiton() { 
    ...
    // if(expedition.progress.attempts<=0) alert('You ran out of attempts. New attempts are added 1 per hour.');
...
function expedition_updateScores({save=0}={}) {
    ...
    $(`#expedition-progress`).text(expedition_obj2string({attempts: expedition.progress.attempts}) + ' left');
...
function expedition_genRewards(state) {
    let mult = state.level;
    let coll = (Math.random() > 0.5) ? {lumber:15*mult} : {stone:16*mult};
...
function expedition_launchNegotiation(state, complete_platform) {
    ...
    // let rewards = (Math.random() > 0.9) ? {attempts:3} : {};
    // if(rewards.attempts) coll = {};
    let {coll, rewards, cost, mult} = expedition_genRewards(state);
...
function expedition_obj2string(obj) {
